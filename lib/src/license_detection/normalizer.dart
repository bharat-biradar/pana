class Normalizer {
  static String normalize(String text) {
    ...
    _equivalentPunctuationMarks.keys.forEach((reg) {
      text = text.replaceAll(reg, _equivalentPunctuationMarks[reg]!);
    });

    //Guideline 8.1.1: Legally equal words must be treated same.
    _varietalWords.keys.forEach((key) {
      text = text.replaceAll(RegExp(key), _varietalWords[key]!);
    });
...
final _ignorableRegex = [
  ...
  RegExp(
      r'^(.{1,5})?copyright (\(c\) )?(\[?\d{4}\]?|y{4})(-\[?\d{4}\]? |-y{4} )?[,.]?.*$',
      caseSensitive: false),
  RegExp(r'^\s*$'),
  RegExp(r'^version \d(\.\d)*(,.*)?$', caseSensitive: false),
  RegExp(r'^(.{1,5})?copyright (\(c\) )?<?year>? <?(copyright )?owners?>?.*$',
      caseSensitive: false),
  RegExp(r'^(.{1,5})?copyright (notice|and permission notice)$',
      caseSensitive: false),
